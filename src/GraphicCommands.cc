...
std::string PointCommand::toString() const {
  std::stringstream s;
  s << getMovePoint().getX() << " " << getMovePoint().getY() << " 1 1 r p2";
  return s.str();
}

std::string MoveCommand::toString() const {
  std::stringstream s;
  s << getMovePoint().getX() << " " << getMovePoint().getY() << " m";
  return s.str();
}

double countDifference(const LeftLineCommand &ll1, const LeftLineCommand &ll2) {
  return countDistanceBetweenConjoinedDirections(ll1.getMovePoint(),
                                                 ll2.getMovePoint());
}

double countDifference(const LeftLineCommand &ll1,
...
double countDifference(const RightLineCommand &rl1,
                       const RightLineCommand &rl2) {
  return countDistanceBetweenConjoinedDirections(rl1.getMovePoint(),
                                                 rl2.getMovePoint());
}
